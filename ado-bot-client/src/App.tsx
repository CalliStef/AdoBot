  ...
  const handleWarningMessage = (message: string | null) => {
    ...
    setTimeout(() => {
      if(warningRef.current) {
        warningRef.current.classList.add("animate-out","slide-out-to-top-40", "fill-mode-forwards")
        warningRef.current.classList.remove("animate-in", "slide-in-from-top-40");
      };
    }, 4000);
  ...
  return (
    <div className="h-screen w-screen relative overflow-hidden">
      ...
      <button className="absolute top-0 left-0 z-10 p-4" onClick={handleLogout}>
        <Icon className="text-[#cad2c5] w-8 h-8" icon="solar:logout-2-outline"/>
      </button> 
      {
          (
            warningMessage &&
            <div ref={warningRef} className="animate-in slide-in-from-top-40 fill-mode-backwards duration-500 delay-500 z-10 flex w-screen absolute top-4 justify-center">
              <p className="bg-red-500 w-[50vw] rounded-md text-white text-md text-center py-2">{warningMessage}</p>
            </div>
          )
      }
      <div className="absolute bottom-0 w-fit h-fit z-[1]">
...

        {
          ...
          : currentChannel ?
        <PostsList channel={currentChannel}/>
          :
          <ChannelsList user={currentUser} handleWarningMessage={handleWarningMessage} handleViewChannel={handleViewChannel}/>
        }
      </div>
